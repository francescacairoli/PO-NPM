...
class BNN_SE():
	...
	def load_train_data(self):
		...
		for iy in range(self.y_dim):
			self.Y_train[:,iy*self.traj_len:(iy+1)*self.traj_len] = C[:,:,iy]

		xmax = np.max(self.X_train, axis = 0)
		ymax = np.max(self.Y_train, axis = 0)
		self.MAX = (xmax, ymax)
		xmin = np.min(self.X_train, axis = 0)
		ymin = np.min(self.Y_train, axis = 0)
		self.MIN = (xmin, ymin)
	...
	def load_test_data(self):
		...
		for iy in range(self.y_dim):
			self.Y_test[:,iy*self.traj_len:(iy+1)*self.traj_len] = C[:,:,iy]
	...
	def evaluate(self, iter_id = None, fld_id = None):
		...
		for k in range(self.n_test_points):
			...
			for i in range(2):
				for j in range(3):
					axs[i,j].plot(tspan, self.X_test_scaled[k, cc*self.traj_len: (cc+1)*self.traj_len], color="blue")
					axs[i,j].plot(tspan, test_mean_pred[k,cc*self.traj_len: (cc+1)*self.traj_len], color="orange")
					axs[i,j].fill_between(tspan, test_mean_pred[k,cc*self.traj_len: (cc+1)*self.traj_len]-test_std_pred[k,cc*self.traj_len: (cc+1)*self.traj_len], test_mean_pred[k,cc*self.traj_len: (cc+1)*self.traj_len]+test_std_pred[k,j*self.traj_len: (j+1)*self.traj_len],alpha = 0.1, color="orange")
					cc += 1
