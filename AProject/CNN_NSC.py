...
def preprocess_inputs(trainX, testX):
	# rescaling in interval [-1,1]
	norm_h = np.max(np.max(trainX, axis = 0),axis=0)/2
...
# define cnn model
def define_model(n_steps, state_dim):
	model = Sequential()
	model.add(Conv1D(32, 4, activation='relu', kernel_initializer='he_uniform', padding='same', input_shape=(n_steps,state_dim)))
...
def summarize_diagnostics(history):
	...
	# save plot to file
	filename = "NSC_"+sys.argv[0].split('/')[-1]
	pyplot.savefig(filename + '_plot.png')
	pyplot.close()

 
# run the test harness for evaluating a model
def run_nsc(train_fn, test_fn, n_eps):
	# load dataset
	...
	print('Plots/NSC Test accuracy > %.2f ' % (acc * 100.0))
	# learning curves
