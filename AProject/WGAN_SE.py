...
class WGAN_SE(object):
	...
	def load_real_data(self):
		...
		umax = np.max(np.max(U, axis = 0),axis = 0)/2
		wmax = np.max(np.max(U, axis = 0),axis = 0)/2
	...
	def generate_latent_points(self, n_samples,  phase, ix = []):
		if phase == "D":
			y_input = self.Y_train[ix], axis = 2
			u_input = self.U_train[ix]
			w_input = self.W_train[ix]
		elif phase == "G":	
	...
	def generate_cond_fake_samples(self, selected_y, selected_u, selected_w, n_samples):
		...
		u_rep = np.tile(selected_u,(n_samples, 1, 1))
		w_rep = np.tile(selected_w,(n_samples, 1, 1))
	...
	def plot_validation_trajectories(self):
		...
		for j in range(n_val_points):
			...
			fig, axs = pyplot.subplots(2,3, figsize = (12,9))

			axs[0,0].plot(tspan, sim_trajectories_unscaled[j,:,0]/13.4775, color="blue")
			axs[0,1].plot(tspan, sim_trajectories_unscaled[j,:,1], color="blue")
			axs[0,2].plot(tspan, sim_trajectories_unscaled[j,:,2], color="blue")
			axs[1,0].plot(tspan, sim_trajectories_unscaled[j,:,3], color="blue")
			axs[1,1].plot(tspan, sim_trajectories_unscaled[j,:,4], color="blue")
			axs[1,2].plot(tspan, sim_trajectories_unscaled[j,:,5], color="blue")
			for traj_idx in range(5):
				axs[0,0].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,0]/13.4775, color="orange")
				axs[0,1].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,1], color="orange")
				axs[0,2].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,2], color="orange")
				axs[1,0].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,3], color="orange")
				axs[1,1].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,4], color="orange")
				axs[1,2].plot(tspan, gen_trajectories_unscaled[j,traj_idx,:,5], color="orange")
			...
			for traj_idx in range(5):
				sns.lineplot(range(n_timesteps), sim_trajectories_unscaled[j,:,sp], color="blue")
				sns.lineplot(range(n_timesteps), gen_trajectories_unscaled[j, traj_idx,:,sp], color="orange")
	...
	def plot_validation_histograms(self):
		...
		leg = ['real', 'gen']
		bins = 20
		sp = 0 # GM = subcutaneous glucose
		for i in range(n_val_points):

			'''
			fig = pyplot.figure()
			xi_val_rep = np.tile(sim_trajectories_unscaled[i,-1,sp], (n_gen_trajs,))
			
			XXX = np.vstack((xi_val_rep, gen_trajectories_unscaled[i,:,-1,sp])).T
			...
			xi_val_rep_2 = np.tile(sim_trajectories_unscaled[i,-1,2], (n_gen_trajs,))
			XXX_2 = np.vstack((xi_val_rep_2, gen_trajectories_unscaled[i,:,-1,2])).T

			xi_val_rep_3 = np.tile(sim_trajectories_unscaled[i,-1,3], (n_gen_trajs,))
			XXX_3 = np.vstack((xi_val_rep_3, gen_trajectories_unscaled[i,:,-1,3])).T
			xi_val_rep_4 = np.tile(sim_trajectories_unscaled[i,-1,4], (n_gen_trajs,))
			XXX_4 = np.vstack((xi_val_rep_4, gen_trajectories_unscaled[i,:,-1,4])).T
			xi_val_rep_5 = np.tile(sim_trajectories_unscaled[i,-1,5], (n_gen_trajs,))
			XXX_5 = np.vstack((xi_val_rep_5, gen_trajectories_unscaled[i,:,-1,5])).T


			axs[0,0].hist(XXX_0, bins = bins, stacked=False, density=False, color=colors, label=leg)
			axs[0,1].hist(XXX_1, bins = bins, stacked=False, density=False, color=colors, label=leg)
			axs[0,2].hist(XXX_2, bins = bins, stacked=False, density=False, color=colors, label=leg)
			axs[1,0].hist(XXX_3, bins = bins, stacked=False, density=False, color=colors, label=leg)
			axs[1,1].hist(XXX_4, bins = bins, stacked=False, density=False, color=colors, label=leg)
			axs[1,2].hist(XXX_5, bins = bins, stacked=False, density=False, color=colors, label=leg)
