...
class Discriminator(nn.Module):
	def __init__(self):
		...
		self.model = nn.Sequential(
			*discriminator_block(opt.x_dim+opt.c_dim, 16),
...
def generate_random_conditions():
	return (np.random.rand(opt.batch_size, opt.c_dim, opt.traj_len)-0.5)*2
...
for iii in range(ds.n_points_test):
	...
	for jjj in range(n_gen_trajs):
		...
		temp_out = generator(Variable(Tensor(z_noise)), Variable(Tensor([ds.C_test_transp[iii]])))
		gen_trajectories[iii,jjj] = temp_out.detach().cpu().numpy()[0]
tspan = range(opt.traj_len)
for kkk in range(ds.n_points_test):
	fig, axs = plt.subplots(2,3, figsize = (12,9))

	axs[0,0].plot(tspan, ds.X_test_transp[kkk,0], color="blue")
	axs[0,1].plot(tspan, ds.X_test_transp[kkk,1], color="blue")
	axs[0,2].plot(tspan, ds.X_test_transp[kkk,2], color="blue")
	axs[1,0].plot(tspan, ds.X_test_transp[kkk,3], color="blue")
	axs[1,1].plot(tspan, ds.X_test_transp[kkk,4], color="blue")
	axs[1,2].plot(tspan, ds.X_test_transp[kkk,5], color="blue")
	
	for traj_idx in range(n_gen_trajs):
		axs[0,0].plot(tspan, gen_trajectories[kkk,traj_idx,0], color="orange")
		axs[0,1].plot(tspan, gen_trajectories[kkk,traj_idx,1], color="orange")
		axs[0,2].plot(tspan, gen_trajectories[kkk,traj_idx,2], color="orange")
		axs[1,0].plot(tspan, gen_trajectories[kkk,traj_idx,3], color="orange")
