...
class SpikingNeuron(object):
	...
	def simple_noisy_sensor(self, full_trajs):
	...
	def generate_labels(self, states, future_horizon):
...
if __name__=='__main__':
	...
	sn = SpikingNeuron()
	trajs = sn.gen_trajectories(100)

	noisy_meas = sn.simple_noisy_sensor(trajs)

	labels = sn.generate_labels(trajs[:,-1], future_horizon=20)

	print(labels, np.sum(labels))


	#fig = plt.figure()
	#plt.plot(np.arange(sn.time_horizon), trajs[0,1])
	#plt.scatter(np.arange(sn.time_horizon), noisy_meas[0])
	#fig.savefig('noise_test_sigma={}.png'.format(sn.noise_sigma))
