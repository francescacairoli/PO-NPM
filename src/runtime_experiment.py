...
runtime_plots = model_name + "/AvgRuntimeExperiments/"
...
onestep_rej = np.zeros(H*n_runtime_points)
twostep_rej = np.zeros(H*n_runtime_points)
nb_onestep_errors = 0
nb_twostep_errors = 0
...
for h in range(H):
	...
	for j in range(n_runtime_points):
		if onestep_pred_errors[j] == -1:
			onestep_rej[h*n_runtime_points+j] = 1	
	

	for jj in range(n_runtime_points):
		if onestep_error_labels[jj] == -1:
			nb_onestep_errors += 1
			if h > half_H:
				if onestep_pred_errors[jj] == -1:
	...
	for k in range(n_runtime_points):
		if twostep_pred_errors[j] == -1:
			twostep_rej[h*n_runtime_points+k] = 1

	for kk in range(n_runtime_points):
		if twostep_error_labels[kk] == -1:
			nb_twostep_errors += 1
			if h > half_H:
				if twostep_pred_errors[kk] == -1:
	...
	if model_name == "AP2":
		state_full = np.array([(state_full[i]+model_class.diff_eq(state_full[i],0)*model_class.dt) for i in range(n_runtime_points)]).reshape((n_runtime_points,1,model_class.state_dim))
		state = state_full[:,:,:model_class.red_state_dim]
		Lt = model_class.gen_labels(state_full[:,0]) 
		if h > H//2:
			meas = model_class.get_noisy_measurments(state, new_sigma=new_sigm)
		else:
			meas = model_class.get_noisy_measurments(state)
	else:
		state = np.array([(state[i,0]+model_class.diff_eq(state[i,0],0)*model_class.dt) for i in range(n_runtime_points)]).reshape((n_runtime_points,1,model_class.state_dim))
		Lt = model_class.gen_labels(state[:,0]) 
		if h > H//2:
			meas = model_class.get_noisy_measurments(state, new_sigma=new_sigm)
		else:
			meas = model_class.get_noisy_measurments(state)

		state_scaled, meas_scaled = scaling(dataset, state, meas)
		Yt = np.concatenate((Yt[:,:,1:], np.transpose(meas_scaled, (0,2,1))), axis = 2)
		St = np.concatenate((St[:,:,1:], np.transpose(state_scaled, (0,2,1))), axis = 2)
		

rej_rate_one = np.hstack((np.sum(onestep_rej[:half_H*n_runtime_points])*np.ones(half_H*n_runtime_points), np.sum(onestep_rej[n_runtime_points*half_H:])*np.ones(half_H*n_runtime_points)))/(half_H*n_runtime_points)
rej_rate_two = np.hstack((np.sum(twostep_rej[:half_H*n_runtime_points])*np.ones(half_H*n_runtime_points), np.sum(twostep_rej[n_runtime_points*half_H:])*np.ones(half_H*n_runtime_points)))/(half_H*n_runtime_points)
...
det_rate_one = np.hstack((term11*np.ones(half_H*n_runtime_points), term12*np.ones(half_H*n_runtime_points)))
det_rate_two = np.hstack((term21*np.ones(half_H*n_runtime_points), term22*np.ones(half_H*n_runtime_points)))


onestep_acc = np.hstack((np.ones(half_H*n_runtime_points)*(half_H*n_runtime_points-len(one_det_errors_1)), np.ones(half_H*n_runtime_points)*(half_H*n_runtime_points-len(one_det_errors_2))))/(half_H*n_runtime_points)
twostep_acc = np.hstack((np.ones(half_H*n_runtime_points)*(half_H*n_runtime_points-len(two_det_errors_1)), np.ones(half_H*n_runtime_points)*(half_H*n_runtime_points-len(two_det_errors_2))))/(half_H*n_runtime_points)

tspan = np.arange(H*n_runtime_points)
fig = plt.figure()
plt.scatter(tspan, rej_rate_one)
plt.plot(tspan, onestep_rej_rate*np.ones(H*n_runtime_points), '--', c='r')
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("rej. rate")
plt.title("runtime (one-step)")
plt.axvline(H*n_runtime_points//2)
fig.savefig(runtime_plots+model_name+"_avg_onestep_rej_rate_sigma={}_H={}.png".format(new_sigm, H))
plt.close()
fig = plt.figure()
plt.scatter(tspan, rej_rate_two)
plt.plot(tspan, twostep_rej_rate*np.ones(H*n_runtime_points), '--', c='r')
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("rej. rate")
plt.title("runtime (two-step)")
plt.axvline(H*n_runtime_points//2)
fig.savefig(runtime_plots+model_name+"_avg_twostep_rej_rate_sigma={}_H={}.png".format(new_sigm, H))
plt.close()
...
plt.plot(tspan, onestep_detection_rate*np.ones(H*n_runtime_points), '--', c='r')
plt.axvline(H*n_runtime_points//2)
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("det. rate")
plt.title("runtime (one-step)")
fig.savefig(runtime_plots+model_name+"_avg_onestep_det_rate_sigma={}_H={}.png".format(new_sigm, H))
plt.close()
fig = plt.figure()
plt.scatter(tspan, det_rate_two)
plt.plot(tspan, twostep_detection_rate*np.ones(H*n_runtime_points), '--', c='r')
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("det. rate")
plt.title("runtime (two-step)")
plt.axvline(H*n_runtime_points//2)
fig.savefig(runtime_plots+model_name+"_avg_twostep_det_rate_sigma={}_H={}.png".format(new_sigm, H))
plt.close()


fig = plt.figure()
plt.scatter(tspan, onestep_acc)
plt.plot(tspan, onestep_test_accuracy*np.ones(H*n_runtime_points), '--', c='r')
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("# PONSC accuracy")
plt.title("runtime (one-step)")
plt.axvline(H*n_runtime_points//2)
fig.savefig(runtime_plots+model_name+"_onestep_accuracy_sigma={}_H={}.png".format(new_sigm, H))
plt.close()
fig = plt.figure()
plt.scatter(tspan, twostep_acc)
plt.plot(tspan, twostep_test_accuracy*np.ones(H*n_runtime_points), '--', c='r')
plt.tight_layout()
plt.xlabel("time")
plt.ylabel("# PONSC accuracy")
plt.title("runtime (two-step)")
plt.axvline(H*n_runtime_points//2)
fig.savefig(runtime_plots+model_name+"_twostep_accuracy_sigma={}_H={}.png".format(new_sigm, H))
plt.close()
