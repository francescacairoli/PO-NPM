...
test_errors = utils.label_correct_incorrect_pred(np.argmax(ponsc_fnc(meas_test), axis=1), output_test)
...
pool_size = 10000
unc_meas, unc_states, unc_outputs = utils.Comb2_PONSC_active_sample_query(pool_size = pool_size, model_class = model, conf_pred = cp_comb_class, trained_svc = query_fnc, se_fnc= se_fnc, dataset=dataset)
...
active_test_errors = utils.label_correct_incorrect_pred(np.argmax(active_ponsc_fnc(meas_test), axis=1), output_test)
...
print("----- ACTIVVE Error detection rate = ", active_detection_rate, "({}/{})".format(active_nb_detected, active_nb_errors))
