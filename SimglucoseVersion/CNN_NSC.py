...
def load_dataset(train_fn, test_fn):
	...
	trainX =  np.expand_dims(train_data["x"], axis=2)
	...
	testX = np.expand_dims(test_data["x"], axis=2)
...
def define_model():
	model = Sequential()
	model.add(Conv1D(32, 4, activation='relu', kernel_initializer='he_uniform', padding='same', input_shape=(80,1)))
